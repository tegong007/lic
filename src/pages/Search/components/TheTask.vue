  ...
  <div class="mt-12vh w-94vw">
    ...
    <div v-for="(value, index) in data" :key="index" class="bg_jianbian mb-2vh flex text-1vw line-height-3vh">
      ...
      <div class="w-27% text-left">
        ...
        <div class="mt-1vh">
          <a-button type="link" class="btn_in mr-1vw" @click="onItemClick('chakan', value)">查看证本列表</a-button>
          <a-button v-if="value.status < 3" type="link" class="btn_in mr-1vw" @click="onItemClick('挂起', value)">挂起</a-button>
          <a-button v-if="value.status === 3 || value.status === 5" type="link" class="btn_in" @click="onItemClick('恢复生产', value)">恢复生产</a-button>
        </div>
      </div>
      <div class="w-40%">
