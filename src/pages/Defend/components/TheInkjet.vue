      ...
      <section v-if="inkjet.positionItems" class="bg_listItem">
        <div class="bgDefend_itemIn" style="display: block">
          <div v-for="(value, index2) in inkjet.positionItems" :key="index2" class="mb-1vh flex items-center">
            <div class="bgDefend_itemIn_tit w-12vw">{{ value.label }}:</div>
            <a-select v-if="value.option.length" v-model:value="formData[value.label + index]" class="w-15vw">
              <a-select-option v-for="option in value.option" :key="option.value" :value="option.value">{{ option.label }}</a-select-option>
            </a-select>
            <a-input v-else v-model:value="formData[value.label + index]" :class="keyInput === value.label ? 'keyInput' : ''" class="w-15vw" placeholder="请输入" :maxlength="3" @click.stop="onInputFocus($event, value.label + index)" />
          </div>
          <div class="bgDefend_itemIn">
            <div class="bgDefend_itemIn_tit w-14vw"></div>
        ...
        <div class="bgDefend_itemIn ml2vw" style="display: block">
          <div v-for="(value, index2) in inkjet.cleanItems" :key="index2" class="mb-1vh flex items-center">
            ...
            <a-select v-model:value="formData[value.label + index]" class="w-15vw">
              <a-select-option v-for="option in value.option" :key="option.value" :value="option.value">{{ option.label }}</a-select-option>
            </a-select>
          </div>
          <div class="bgDefend_itemIn">
            <div class="bgDefend_itemIn_tit w-12vw"></div>
      ...
      <section v-if="inkjet.printItems && inkjet.printItems.length > 0" class="bg_listItem">
        <div class="bgDefend_itemIn">
          <div v-for="(value, index2) in inkjet.printItems" :key="index2" class="flex items-center">
            <div class="bgDefend_itemIn_tit w-15vw">{{ value.label }}:</div>
