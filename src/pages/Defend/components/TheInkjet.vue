      ...
      <section v-if="inkjet.positionItems" class="bg_jianbian mb-2vh ml-2vw flex">
        <div class="mr-3vw flex items-center">
          <div v-for="(value, index2) in inkjet.positionItems" :key="index2" class="flex items-center">
            ...
            <a-input v-else v-model:value="formData[value.label]" :class="keyInput === value.label ? 'keyInput' : ''" class="w-10vw" placeholder="请输入" :maxlength="15" @click.stop="onInputFocus($event, ['systemConfig', 'ipLocal'], 15)" />
          </div>
          <a-button type="link" class="btn_search ml-1vw" @click="transferApi('/uvpdps/moto-reposition', index, inkjet)">复位</a-button>
          <a-button type="link" class="btn_search ml-1vw" @click="transferApi('/ips-r/read-card-uid', index, inkjet)">移动</a-button>
        </div>
      </section>
      <section class="bg_jianbian mb-2vh ml-2vw flex">
        <div class="mr-3vw flex items-center">
          <div v-for="(value, index2) in inkjet.cleanItems" :key="index2" class="flex items-center">
            <div class="ml-2vw w-8vw">{{ value.label }}:</div>
            <a-select v-model:value="formData[value.label]" class="w-10vw">
              <a-select-option v-for="option in value.option" :key="option.value" :value="option.value">{{ option.label }}</a-select-option>
            </a-select>
          </div>
          <a-button type="link" class="btn_search ml-1vw" @click="transferApi('/ips-r/read-test-data', index, inkjet)">清洗</a-button>
        </div>
      </section>
      <section class="bg_jianbian mb-2vh ml-2vw flex">
        <div class="mr-3vw flex items-center">
          ...
          <a-button type="link" class="btn_search ml-1vw" @click="transferApi('/ips-r/read-test-data', index, inkjet)">打印测试页</a-button>
        </div>
      </section>
    </template>
  </div>
...
async function transferApi(url: string, index: any, inkjetObj = { deviceIndex: null, dev: '' }, inputData = {}) {
  console.log(formData.value, index, readerObj);
  try {
    useAppStore().setSpinning(true);
    const params = { transURI: url, paraIn: { objs: [{ deviceIndex: inkjetObj.deviceIndex, dev: inkjetObj.dev, data: inputData }] } };
  ...
  /* try {
    ...
    const data: any = await defendModule.getApiTransfer(params);
    if (data.rslts[0].code === 0) {
      if (props.updateItem) props.updateItem('readers', index, url === '/ips-r/read-test-data' ? data.rslts[0].data : data.rslts[0].cardUid);
      if (url === '/ips-r/write-test-data') notification.success({ message: '成功', description: '操作成功', placement: 'bottomRight', class: 'notificationE-custom-class' });
    } else {
      throw data.rslts[0].msg || '未知错误';
    }
  } catch (error) {
