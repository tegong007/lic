<template>
  <div class="bgDefend_item no-first-bar">
    <template v-for="(item, idx) in (cur.ocrBlank || [])" :key="idx">
      <div class="bgDefend_tit">OCR空白页检测<span v-if="(cur.ocrBlank || []).length > 1">{{ idx + 1 }}</span></div>
      <section class="bg_listItem" style="display: block">
        <div style="display: flex; flex-wrap: wrap; gap: 1vh">
          ...
          <div class="bgDefend_itemIn">
            ...
            <a-input v-model:value="item.usExposureTime" :class="keyInput === `ocrBlank.${idx}.usExposureTime` ? 'keyInput' : ''" class="w-9vw" placeholder="62~9999764" :maxlength="12" @click.stop="onInputFocus($event, `ocrBlank.${idx}.usExposureTime`, 12)" />
